...
#ifndef __TRANCE__MODEL__MODEL4__HPP__
#define __TRANCE__MODEL__MODEL4__HPP__ 1
...
namespace trance
{
  namespace model
  {
    ...
    public:
      Model4() : Model() {}
      Model4(const path_type& path) { read(path); }
      ...
      Model4& operator-=(const Model4& x);
      Model4& operator*=(const double& x);
      Model4& operator/=(const double& x);
...
{
  inline
  void swap(trance::model::Model4& x, trance::model::Model4& y)
  {
    x.swap(y);
  }
};
