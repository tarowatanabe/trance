...
#ifndef __TRANCE__MODEL__MODEL1__HPP__
#define __TRANCE__MODEL__MODEL1__HPP__ 1
...
namespace trance
{
  namespace model
  {
    ...
    public:
      Model1() : Model() {}
      Model1(const path_type& path) { read(path); }
      ...
      Model1& operator-=(const Model1& x);
      Model1& operator*=(const double& x);
      Model1& operator/=(const double& x);
	...
	const double range_sh = std::sqrt(6.0 / (hidden_ + embedding_));
...
{
  inline
  void swap(trance::model::Model1& x, trance::model::Model1& y)
  {
    x.swap(y);
  }
};
