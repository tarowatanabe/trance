...
#ifndef __TRANCE__MODEL__MODEL8__HPP__
#define __TRANCE__MODEL__MODEL8__HPP__ 1
...
namespace trance
{
  namespace model
  {
    ...
    public:
      Model8() : Model() {}
      Model8(const path_type& path) { read(path); }
      ...
      Model8& operator-=(const Model8& x);
      Model8& operator*=(const double& x);
      Model8& operator/=(const double& x);
...
{
  inline
  void swap(trance::model::Model8& x, trance::model::Model8& y)
  {
    x.swap(y);
  }
};
