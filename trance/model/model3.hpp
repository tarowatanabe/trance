...
#ifndef __TRANCE__MODEL__MODEL3__HPP__
#define __TRANCE__MODEL__MODEL3__HPP__ 1
...
namespace trance
{
  namespace model
  {
    ...
    public:
      Model3() : Model() {}
      Model3(const path_type& path) { read(path); }
      ...
      Model3& operator-=(const Model3& x);
      Model3& operator*=(const double& x);
      Model3& operator/=(const double& x);
...
{
  inline
  void swap(trance::model::Model3& x, trance::model::Model3& y)
  {
    x.swap(y);
  }
};
