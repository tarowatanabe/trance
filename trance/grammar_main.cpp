...
int main(int argc, char** argv)
{
  ...
  trance::Grammar grammar(argv[1]);
  
  boost::shared_ptr<trance::Signature> signature(trance::Signature::create(argc == 3 ? argv[2] : "none"));
  ...
  trance::Symbol symbol;
  while (std::cin >> symbol) {
    if (symbol.non_terminal()) {
      const trance::Grammar::rule_set_type& rules = grammar.unary(symbol);
      
      if (rules.empty()) 
	std::cout << "no rule..." << std::endl;
      else
	std::copy(rules.begin(), rules.end(), std::ostream_iterator<trance::Rule>(std::cout, "\n"));
    } else {
      const trance::Grammar::rule_set_type& rules = grammar.preterminal(*signature, symbol);
