...
#ifndef __TRANCE__OBJECTIVE__MARGIN_EARLY__HPP__
#define __TRANCE__OBJECTIVE__MARGIN_EARLY__HPP__ 1
	...
	if (option.margin_all_) {
	  ...
	  for (size_type c = 0; c != kbest_candidate_size; ++ c)
		...
		if (state_candidate_early && state_oracle_early) {
		  ...
		  backward_[state_candidate_early].loss_ += loss_factor;
		  backward_[state_oracle_early].loss_    -= loss_factor;
	  ...
	  if (! state_candidate_early || ! state_oracle_early) return 0.0;
	  
	  backward_[state_candidate_early].loss_ += 1.0;
	  backward_[state_oracle_early].loss_    -= 1.0;
	  
	  states_[state_candidate_early.step()].insert(state_candidate_early);
	  states_[state_oracle_early.step()].insert(state_oracle_early);
