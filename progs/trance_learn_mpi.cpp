...
#include <trance/objective/margin_derivation.hpp>
#include <trance/objective/margin_evalb.hpp>
#include <trance/objective/margin_early.hpp>
#include <trance/objective/margin_late.hpp>
#include <trance/objective/margin_max.hpp>
#include <trance/objective/violation_early.hpp>
#include <trance/objective/violation_late.hpp>
#include <trance/objective/violation_max.hpp>

#include <trance/optimize/adagrad.hpp>
#include <trance/optimize/adadec.hpp>
#include <trance/optimize/adadelta.hpp>
#include <trance/optimize/sgd.hpp>
...
int main(int argc, char** argv)
{
  ...
  try {
	...
	throw std::runtime_error("no model file? " + model_file.string());
      ...
      case trance::model::MODEL1: model_model1 = true; break;
      case trance::model::MODEL2: model_model2 = true; break;
	...
	std::cerr << "model1" << std::endl;
      ...
      trance::model::Model1 theta(hidden_size, embedding_size, grammar);
	...
	std::cerr << "model2" << std::endl;
      ...
      trance::model::Model2 theta(hidden_size, embedding_size, grammar);
	...
	std::cerr << "model3" << std::endl;
      ...
      trance::model::Model3 theta(hidden_size, embedding_size, grammar);
	...
	std::cerr << "model4" << std::endl;
      ...
      trance::model::Model4 theta(hidden_size, embedding_size, grammar);
...
struct Task
{
  ...
  typedef typename trance::model_traits<Theta>::model_type    model_type;
  typedef typename trance::model_traits<Theta>::gradient_type gradient_type;

  typedef trance::Loss loss_type;
  ...
  typedef trance::Evalb       evalb_type;
  typedef trance::EvalbScorer scorer_type;
  ...
  trance::Parser       parser_;
  trance::ParserOracle parser_oracle_;
  ...
  void operator()()
  {
    ...
    trance::Parser::derivation_set_type candidates;
    trance::Parser::derivation_set_type oracles;

    trance::Derivation derivation;
	   ...
	   Gen& gen)
{
  ...
  if (option.margin_derivation())
    learn(optimizer, trance::objective::MarginDerivation(), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.margin_evalb())
    learn(optimizer, trance::objective::MarginEvalb(), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.margin_early())
    learn(optimizer, trance::objective::MarginEarly(), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.margin_late())
    learn(optimizer, trance::objective::MarginLate(), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.margin_max())
    learn(optimizer, trance::objective::MarginMax(), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.violation_early())
    learn(optimizer, trance::objective::ViolationEarly(), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.violation_late())
    learn(optimizer, trance::objective::ViolationLate(), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.violation_max())
    learn(optimizer, trance::objective::ViolationMax(), option, trees, tests, grammar, signature, feats, theta, gen);
  else
	   ...
	   Gen& gen)
{
  ...
  if (option.optimize_adagrad())
    learn(trance::optimize::AdaGrad<Theta>(theta, option), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.optimize_adadec())
    learn(trance::optimize::AdaDec<Theta>(theta, option), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.optimize_adadelta())
    learn(trance::optimize::AdaDelta<Theta>(theta, option), option, trees, tests, grammar, signature, feats, theta, gen);
  else if (option.optimize_sgd())
    learn(trance::optimize::SGD<Theta>(theta, option), option, trees, tests, grammar, signature, feats, theta, gen);
  else
...
{
  if (MPI::COMM_WORLD.Get_rank() == 0)
    learn_root(optimizer, objective, option, trees, tests, grammar, signature, feats, theta, gen);
  else
    learn_others(optimizer, objective, option, grammar, signature, feats, theta);
}
