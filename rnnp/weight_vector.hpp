...
#ifndef __RNNP__WEIGHT_VECTOR__HPP__
#define __RNNP__WEIGHT_VECTOR__HPP__ 1
...
namespace rnnp
{
  ...
  {
  public:
    typedef rnnp::Feature feature_type;
    typedef rnnp::Feature key_type;
  ...
  std::istream& operator>>(std::istream& is, WeightVector<T,A>& x)
  {
    ...
    while ((is >> feature) && (is >> value))
      x[rnnp::Feature(feature).id()] = value;
...
namespace std
{
  ...
  template <typename T, typename A>
  inline
  void swap(rnnp::WeightVector<T,A>& x, rnnp::WeightVector<T,A>& y)
  {
    x.swap(y);
  }
...
#include <rnnp/feature_vector_compact.hpp>
#include <rnnp/feature_vector_linear.hpp>
