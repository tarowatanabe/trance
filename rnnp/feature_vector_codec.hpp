...
#ifndef __RNNP__FEATURE_VECTOR_CODEC__HPP__
#define __RNNP__FEATURE_VECTOR_CODEC__HPP__ 1
...
namespace rnnp
{
  ...
  struct FeatureVectorCODEC
  {
    ...
    void encode(const FeatureVector<Tp, Alloc>& x, Iterator result)
    {
      ...
      buffer_.clear();
      buffer_.reserve(x.size() * 16);
      buffer_.resize(x.size() * 16);
      
      std::copy(buffer_.begin(), encoder(x.begin(), x.end(), buffer_.begin()), result);
    }
