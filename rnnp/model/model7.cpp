...
namespace rnnp
{
  namespace model
  {
			    ...
			    const grammar_type& grammar)
    {
      ...
      Wre_ = tensor_type::Zero(hidden_ * vocab_category_.size(), hidden_ + hidden_ + hidden_ + hidden_ + hidden_ + hidden_);
    ...
    void Model7::write(const path_type& path) const
    {
      ...
      Model::write_category(rep.path("Wre.txt.gz"), rep.path("Wre.bin"), Wre_, hidden_, hidden_ + hidden_ + hidden_ + hidden_ + hidden_ + hidden_);
	...
	throw std::runtime_error("invalid dimension");
      ...
      Wre_ = tensor_type::Zero(Wre_.rows(), hidden_ + hidden_ + hidden_ + hidden_ + hidden_ + hidden_);
      ...
      Model::read_category(rep.path("Wre.txt.gz"), rep.path("Wre.bin"), Wre_, hidden_, hidden_ + hidden_ + hidden_ + hidden_ + hidden_ + hidden_);
...
#define MODEL_STREAM_OPERATOR(Theta, OpEmbedding, OpCategory, OpWeights, OpMatrix, Stream) \
    ...
    Theta.OpCategory(Stream, Theta.Bsh_, Theta.hidden_, 1);		\
									\
    Theta.OpCategory(Stream, Theta.Wre_, Theta.hidden_, Theta.hidden_ + Theta.hidden_ + Theta.hidden_ + Theta.hidden_ + Theta.hidden_ + Theta.hidden_); \
