...
#ifndef __TMT__OBJECTIVE__MARGIN_MAX__HPP__
#define __TMT__OBJECTIVE__MARGIN_MAX__HPP__ 1
	...
	for (size_type c = 0; c != kbest_candidate_size; ++ c)
	  if (error_[c])
	    for (size_type o = 0; o != kbest_oracle_size; ++ o) {
	      ...
	      node_type node_candidate_max;
	      node_type node_oracle_max;
		...
		if (node_candidate.state().step() > node_oracle.state().step())
		  ...
		  if (suffered && error > error_max) {
		    error_max = error;
		    node_candidate_max = node_candidate;
		    node_oracle_max    = node_oracle;
		  }
	      ...
	      if (node_candidate_max && node_oracle_max) {
		...
		backward_type& backward_candidate = backward_[node_candidate_max.state()];
		backward_type& backward_oracle    = backward_[node_oracle_max.state()];
